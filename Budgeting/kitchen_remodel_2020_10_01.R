...
existing.credit.card = function(date){
  ...
  payment = switch(as.character(date),
                   "2020-10-15" = 3686, # Citi card existing charges
                   "2020-10-20" = 1126, # SLFCU card existing charges + any new charges between 2020/10/05 and 2020/10/20 (leaving latter as zero and putting all to new Citi charges)
                   0)
  return(payment)
}
...
ggplot(plot.df, aes(date, balance)) + 
  ...
  geom_hline(yintercept = 15000, color = "purple") +
