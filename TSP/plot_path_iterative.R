...
# Creating path data
shortest.path.df = read.csv("shortest_path_iterative.csv")
coord.df = read.csv("state_centroids.csv")

shortest.path.df = shortest.path.df %>%
  left_join(coord.df, by = "name") %>%
  rename(lat = latitude, long = longitude) %>%
  filter(iteration %in% floor(seq(min(iteration), max(iteration), length.out = 250))) %>%
  group_by(iteration) %>%
  mutate(step = 1:n(), path.length = max(cumulative.distance))
